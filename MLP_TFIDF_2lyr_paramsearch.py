...
class DeepMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim1=100, hidden_dim2=50, output_dim=6, dropout_rate=0.5):
...
def objective(params):
    # Извлекаем гиперпараметры из пространства поиска
    lr = params['lr']
    hidden_dim1 = int(params['hidden_dim1'])
    hidden_dim2 = int(params['hidden_dim2'])
    dropout_rate = params['dropout_rate']

    print(f"\nОптимизируем: lr={lr:.5f}, hidden_dim1={hidden_dim1}, hidden_dim2={hidden_dim2}, dropout_rate={dropout_rate:.3f}")

    n_splits = 5
    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
    fold_accuracies = []

    # Предполагается, что train_labels и X_train уже определены глобально
    labels_arr = np.array(train_labels)

    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels_arr)), labels_arr), start=1):
        print(f"\n--- Fold {fold}/{n_splits} ---")
        
        # Формируем подвыборки для текущего фолда
        X_tr_fold = X_train[train_idx]
        y_tr_fold = labels_arr[train_idx]
        X_val_fold = X_train[val_idx]
        y_val_fold = labels_arr[val_idx]
        
        # Создаем объекты Dataset и DataLoader для фолда
        train_dataset_fold = TextDataset(X_tr_fold, y_tr_fold)
        val_dataset_fold = TextDataset(X_val_fold, y_val_fold)
        train_loader_fold = DataLoader(train_dataset_fold, batch_size=BATCH_SIZE, shuffle=True)
        val_loader_fold = DataLoader(val_dataset_fold, batch_size=BATCH_SIZE, shuffle=False)
        
        # Инициализируем модель с оптимизируемыми гиперпараметрами
        model = DeepMLP(input_dim, hidden_dim1=hidden_dim1, hidden_dim2=hidden_dim2, 
                        output_dim=num_classes, dropout_rate=dropout_rate).to(device)
        
        # Первый этап обучения с использованием глобальных параметров (например, с patience)
        print("\n--- Training improved model (Этап 1) ---")
        model = train_model(model, train_loader_fold, val_loader_fold, epochs=EPOCHS, lr=LR, device=device, patience=PATIENCE)
        
        # Второй этап обучения с оптимизируемым lr на EPOCHS_OPT эпох (для ускорения оптимизации)
        print("\n--- Training with EPOCHS_OPT (Этап 2) ---")
        model = train_model(model, train_loader_fold, val_loader_fold, epochs=EPOCHS_OPT, lr=lr, device=device)
        
        # Оценка модели на валидационной выборке для текущего фолда
        model.eval()
        all_preds, all_true = [], []
        with torch.no_grad():
    ...
    # Усредняем точность по всем фолдам
...
space = {
    ...
    'hidden_dim1': hp.quniform('hidden_dim1', 10, 100, 1),               # целые значения от 10 до 100
    'hidden_dim2': hp.quniform('hidden_dim2', 10, 100, 1), 
...
final_model = DeepMLP(input_dim, hidden_dim1=best_hidden_dim1, hidden_dim2=best_hidden_dim2, 
                output_dim=num_classes, dropout_rate=best_dropout).to(device)
