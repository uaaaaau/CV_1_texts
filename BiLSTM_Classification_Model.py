...
# ПАРАМЕТРЫ
CHUNK_SIZE = 500    # максимальная длина чанка в токенах
...
HIDDEN_DIM = 128    # скрытый размер LSTM (для bidirectional будет 2*HIDDEN_DIM на выходе)
...
class BiLSTMClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_classes,
        ...
        self.lstm = nn.LSTM(embed_dim,
                            ...
                            batch_first=True,
                            bidirectional=True)
        # Поскольку LSTM двунаправленная, на выходе у нас будет размер 2*hidden_dim
...
def dynamic_chunk_params(text, factor=3, step_fraction=0.5):
    # Разбиваем текст на предложения по знакам окончания (точка, восклицание, вопрос)
    ...
    # Размер чанка: 75-й процентиль, умноженный на фактор
...
def predict_text_dynamic(model, text, word2idx, device='cpu'):
    """
    Использует функцию dynamic_chunk_params, чтобы
    адаптивно выбирать chunk_size и step_size для каждого текста.
    """
    chunk_size, step_size = dynamic_chunk_params(text)
    return predict_text(model, text, word2idx, chunk_size, step_size, device)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(csv_path):
        ...
        test_true = []
        test_pred = []
        print("\n--- Классификация тестовых файлов (BiLSTM) ---")
        ...
        # Классифицируем 21 тестовый отрывок с динамическими параметрами
        test_true_dynamic = []
        test_pred_dynamic = []
        print("\n--- Классификация тестовых файлов (динамические параметры, BiLSTM) ---")
        for _, row in df.iterrows():
            fname = os.path.join(test_files_dir, row['filename'])
            true_author = row['author']
            if not os.path.exists(fname):
            ...
            text = read_file(fname)
            pred_label = predict_text_dynamic(model, text, word2idx, device=device)
            pred_author = label2author[pred_label]
            
            test_true_dynamic.append(true_author)
            test_pred_dynamic.append(pred_author)
            
            print(f"Файл {fname} -> предсказанный автор: {pred_author} (истинный: {true_author})")

        print("\n=== Оценка (динамические параметры, BiLSTM) ===")
        print("Accuracy:", accuracy_score(test_true_dynamic, test_pred_dynamic))
        print("\nConfusion Matrix:")
        print(confusion_matrix(test_true_dynamic, test_pred_dynamic, labels=list(label2author.values())))
        print("\nClassification Report:")
        print(classification_report(test_true_dynamic, test_pred_dynamic,
                                    labels=list(label2author.values())))

        # Примеры ошибок
        print("\nПримеры ошибок на тестовой выборке (динамические параметры):")
        for true_author, pred_author, fname in zip(test_true_dynamic, test_pred_dynamic, df['filename']):
            if true_author != pred_author:
                print(f"Файл: {fname} | Истинный автор: {true_author} | Предсказано: {pred_author}")
